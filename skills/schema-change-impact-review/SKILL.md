---
name: schema-change-impact-review
description: Use this skill when reviewing a proposed dataset schema change with DataHub lineage, metadata, deterministic risk evidence, migration safeguards, and an explicitly confirmed root-only write-back. Trigger for column rename, drop, addition, type change, downstream impact, merge safety, or schema migration review requests.
---
...
- A reachable DataHub OSS or DataHub Cloud instance with lineage already
  ingested.
- A configured DataHub client or read-only MCP/Agent Context Kit connection.
- For the Python Agent Context Kit path, `datahub-agent-context` and
  `acryl-datahub` installed with `DataHubClient.from_env()` configured.
...
- A sanitized Agent Context execution trace with requested tools, successes,
...
Use the available DataHub read interface. With Agent Context Kit 1.6.0.17 or a
compatible release:

```python
from datahub_agent_context.context import DataHubContext
from datahub_agent_context.mcp_tools.entities import (
    get_entities,
    list_schema_fields,
)

with DataHubContext(client):
    entity = get_entities(urns=[root_urn])
    fields = list_schema_fields(
        urn=root_urn,
        keywords=[affected_column],
        limit=100,
    )
```
...
safeguards, and the Agent Context trace. The narrative must state that the risk
...
After valid confirmation, write only the reviewed root asset through the
approved write-back adapter. Preserve unrelated documentation and replace or
...
- **Agent Context Kit unavailable or timed out:** return an honest degraded or
...
Validate the skill structure with the Agent Skills validator used by the host
environment. In this repository:

```powershell
python C:\Users\ru765\.codex\skills\.system\skill-creator\scripts\quick_validate.py `
  skills\schema-change-impact-review
```

Then run the LineageShield tests and a read-only live scenario. Verify that the
trace lists `get_entities` and `get_lineage`, fallback is visible when used,
the deterministic decision is unchanged, and no DataHub mutation occurs until
the separate confirmed Apply action.
